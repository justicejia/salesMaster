package com.sohu.focus.salesmaster.kernal.http.cookie;
...
public class PersistentCookieStore implements CookieStore {
    ...
    private static final String COOKIE_PREFS = "focus_sales_cookie";        //cookie使用prefs保存
    ...
    public PersistentCookieStore() {
        cookiePrefs = BaseApplication.getApplication().getSharedPreferences(COOKIE_PREFS, Context.MODE_PRIVATE);
        cookies = new HashMap<>();
    ...
    public void saveCookie(HttpUrl url, Cookie cookie) {
        saveCookie(url,cookie,true);
    }

    @Override
    public void saveCookie(HttpUrl url, Cookie cookie, boolean isNeedShareSave) {
        if (!cookies.containsKey(url.host())) {
            cookies.put(url.host(), new ConcurrentHashMap<String, Cookie>());
        }
        //当前cookie是否过期
        if (isCookieExpired(cookie)) {
            ...
            saveCookie(url, cookie, getCookieToken(cookie), isNeedShareSave);
        }
    }

    /**
     * 临时方法，保存cookie，并将cookies持久化到本地（可选）,数据结构为
     * Url.host -> Cookie1.name,Cookie2.name,Cookie3.name
     * cookie_Cookie1.name -> CookieString
     * cookie_Cookie2.name -> CookieString
     */
    private void saveCookie(HttpUrl url, Cookie cookie, String name, boolean isNeedShareSave) {
        //内存缓存
    ...
    private void saveCookie(HttpUrl url, Cookie cookie, String name) {
        saveCookie(url,cookie,name,true);
    }
    ...
    private String encodeCookie(SerializableHttpCookie cookie) {
        ...
        try {
            ...
            Logger.ZQ().d(LOG_TAG, "IOException in encodeCookie", e);
            return null;
        }
    ...
    private Cookie decodeCookie(String cookieString) {
        ...
        try {
            ...
            Logger.ZQ().d(LOG_TAG, "IOException in decodeCookie", e);
        } catch (ClassNotFoundException e) {
            Logger.ZQ().d(LOG_TAG, "ClassNotFoundException in decodeCookie", e);
        }
        return cookie;
    }
